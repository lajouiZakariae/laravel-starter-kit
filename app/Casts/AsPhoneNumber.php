...
class AsPhoneNumber implements CastsAttributes {
    ...
    public function get(Model $model, string $key, mixed $value, array $attributes): ?PhoneNumber {
        ...
        if (blank($value) && blank($countryCode)) {
            return null;
        }
        ...
        if (blank($value)) {
            throw new InvalidArgumentException('The phone number must be a non-empty string');
        }

        if (blank($countryCode)) {
            throw new InvalidArgumentException('The country code must be a non-empty string');
        }
    ...
    public function set(Model $model, string $key, mixed $value, array $attributes): array {
        ...
        return [
            'phone_number_country_code' => $value->iso2CountryCode,
